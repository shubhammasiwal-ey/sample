...
export const SchemeMaster = () => {
  ...
  const schemeOptions = useMemo(
    () =>
      schemes.map((s: any) => ({
        ...
        value: s.scheme_code, // match trigger field
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      let requiredDocsParsed: any[] = [];

      try {
        requiredDocsParsed = requiredDocuments.documents;
        if (!requiredDocsParsed.length) {
          toastRef.current?.show({
            severity: "error",
            summary: "Error",
        ...
        toastRef.current?.show({
          severity: "error",
          summary: "Error",
          detail: "Invalid Required Documents configuration",
        });
        setActiveTabIndex(4);
        return;
      }
      ...
      if (
        !Array.isArray(requiredDocsParsed) ||
        requiredDocsParsed.length === 0
      ) {
        toastRef.current?.show({
          severity: "error",
          summary: "Error",
  ...
  const handleEdit = useCallback((scheme: Scheme) => {
    ...
    setRequiredDocuments({
      documents: Array.isArray(reqDocsArray) ? reqDocsArray : [],
    });
