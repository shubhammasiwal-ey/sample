...
import {
    useJDInspectionChecklists,
    useJDCreateInspectionChecklist,
    useJDDeleteInspectionChecklist,
    InspectionChecklist
...
// Main Component
// ===================================
export default function JDChecklistsPage() {
    const { user } = useAuth();
    // Default to 'all' ONLY if user deptId missing (fallback), else use user.deptId
    // Actually we want to fetch ONLY my department's services
    const userDeptId = user?.deptId;

    const { data: checklists = [], isLoading: checklistsLoading } = useJDInspectionChecklists();
    const { data: services = [], isLoading: servicesLoading } = useServices(
        userDeptId ? { departmentIds: [userDeptId] } : { isActive: true }
    );
    // No need to fetch all departments anymore for filtering

    const createChecklist = useJDCreateInspectionChecklist();
    const deleteChecklist = useJDDeleteInspectionChecklist();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await createChecklist.mutateAsync({
                ...
                version: '1.0', // Auto-assign version 1.0 based on user request
        ...
        <div className="container-fluid p-4" style={{ backgroundColor: '#f8fafc', minHeight: '100vh' }}>
            ...
            <div className="d-flex justify-content-between align-items-start mb-4">
                <div>
                    ...
                    <p className="text-muted mb-0">Manage inspection checklists for your department</p>
                </div>
                <button
                                        ...
                                        <tr key={checklist.id}>
                                            ...
                                            <td className="text-center px-4">
                                                <div className="d-flex justify-content-center gap-2">
                                                    <Link
                                                        href={`/investor/inspections/jd-portal/checklists/${checklist.id}`}
                                    ...
                                    <div className="row g-3 mb-4">
                                        <div className="col-md-12">
                                            ...
                                            <small className="text-muted d-block mt-1">
                                                Listing services for your department only.
                                            </small>
                                        </div>
                                    </div>
