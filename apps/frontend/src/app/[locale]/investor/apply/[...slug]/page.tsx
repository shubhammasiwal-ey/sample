...
export default function ApplicationFormPage() {
  ...
  const hasRequiredDocuments =
    Array.isArray(scheme?.required_documents?.documents) &&
    scheme.required_documents.documents.some(
      (doc: any) => doc.is_mandatory === true,
    );
  ...
  const saveToIncentiveSubmission = async (
    ...
    try {
      ...
      const dto = {
        userId: 1,
        incentiveId: scheme.id,
        departmentId: 1,
        postData: JSON.stringify(currentValues), // ✅ FIX
      ...
      await apiClient.post("/incentive-application-submission", dto);
      ...
      toastRef.current?.show({
        severity: "error",
        summary: "Error",
        detail: err.response?.data?.message || "Failed to save form data",
        life: 3000,
      });
    } finally {
  ...
  const handleNext = async () => {
    ...
    if (hasErrors) {
      if (!validate(currentStepSections, fieldMaster)) {
        toastRef.current?.show({
          severity: "error",
    ...
    if (draftUrl) {
      ...
      await saveToIncentiveSubmission(values, activeStep);
    }
  ...
  const handleFinalNext = async () => {
    ...
    // Redirect to document upload if required
    if (hasRequiredDocuments && scheme) {
      ...
      router.push(
        `/investor/apply/documents/${policyCode}/${scheme.scheme_code}/v${scheme.version}`,
      );
      return;
    }

    // No documents → submit directly
  ...
  return (
    <div className="p-6">
      ...
      <div
        className={
          isPopupEnabled && showPrePopup ? "pointer-events-none opacity-50" : ""
        }
      >
          ...
          {isMultiStep ? (
            ...
            currentStepSections.length > 0 && (
              <DynamicFormRenderer
                ...
                errors={errors}
                requiredDocuments={values.required_documents} // <-- new prop
              />
            )
          ) : (
